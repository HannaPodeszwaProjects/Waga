...
state Rozwiaz::rozwiaz_gracz(Plansza& p)
{
	COORD c;
	c.X = 2;
	c.Y = 7;
...
state Stan::do_rozwiaz()
{
	...
	for (int i = 0;i < 9;i++)
		for (int j = 0;j < 9;j++)
		{
			...
			{
				Pole nowe_pole(-1, false, (2 + 4 * i), (7 + j * 2));
...
bool Rozwiaz::rozwiaz(Plansza &plansza_stan)
{
	int wiersz, kolumna;

	if (!znajdz_puste(plansza_stan,wiersz,kolumna))
		return true;

	for (int i = 1;i <= 9;i++)
	{
		if (czy_moze(plansza_stan,wiersz, kolumna, i))
		{
...
bool Rozwiaz::czy_moze(Plansza plansza_stan,int wiersz, int kolumna, int i)
{
...
bool Rozwiaz::znajdz_puste(Plansza plansza_stan,int &wiersz, int &kolumna)
{
...
bool Rozwiaz::uzyj_wiersz(Plansza plansza_stan,int wiersz, int nr)
{
	for (int i = 0;i < 9;i++)
	{
...
bool Rozwiaz::uzyj_kolumna(Plansza plansza_stan, int kolumna, int nr)
{
	for (int i = 0;i < 9;i++)
	{
...
bool Rozwiaz::uzyj_kwadrat(Plansza plansza_stan, int wiersz, int kolumna, int nr)
{
	for (int i = 0;i < 3;i++)
		for (int j = 0;j < 3;j++)
		{
